...
2. Create a jenkins user named yousuf with the passwordBruCStnMT5. Their full name should match Yousuf.
...
By default jenkins haven't install `Matrix Authorization Strategy` plugin. So we have to install that befor do this. Search for `matrix-auth` in `Available plugins` section.