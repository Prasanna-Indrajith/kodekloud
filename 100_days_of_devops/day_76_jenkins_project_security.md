...
3.  **Give overall read permissios to users**
    - `Manage jenkins` > `Security`
    - Authorization : **Project base Metrics authorization stratergy**
      - `add user`
        - rohan : Overall read
        - sam : Overall read
4.  **Give project permissions**

    - `Dashboard` > `Packages` > `Configure`
    - Enable project-based security
      - Add user : sam, rohan
      - Give permissions as they say

5.  **Verification**

    - Logout and login as `sam` and `rohand` with given passwords and check their permitted works
